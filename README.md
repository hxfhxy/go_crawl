...
- 增量去重会跳过已入库页面、连带放弃其出链，跨运行的链接发现需要 links 表支持（当前用换关键词/删库兜底，见 LEARNING.md 第 8 节）。
- 参数不同的模板页（如 `?category=1/2/3`）视为不同 URL，尚未做 URL 规范化聚类。

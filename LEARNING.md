...
| `stats` | 关键词词频统计 | store |
| `main` | 组装、命令行、退出信号 | config + scheduler + stats |
...
### 统计层要点（`stats/stats.go`）

- **接口演进的实战案例**：`Store` 接口最初只有 `Save/Seen/Close`，统计需要读全量数据，于是给它加了 `All()` 方法——**两份实现各自补齐，调度器代码一行没动**。给既有接口加方法时，“实现方各自补齐”就是这个模式的日常。
- **统计策略**：文本统一 `strings.ToLower` 后用 `strings.Count` 对关键词做子串计数（关键词清单都是小写）。比“分词 + map 计数”粗糙，但对英文技术词足够准、代码量小——**先选够用的方案**。
- **排序细节**：`sort.Slice` 按次数降序，次数相同按字典序升序（tie-break），保证输出稳定可复现。

### 增量去重与链接发现的死角

一个值得记录的设计教训：跳过一个已入库的页面，等于同时放弃了它解析出的所有链接。极端情况：唯一任务（种子页）已在库中 → 被跳过 → 队列空 → 整个爬虫空转，**链接发现和增量去重打架了**。目前的兜底是换关键词或删库重爬；根治方案是加一张 `links` 表持久化每个页面的出链，让前沿队列可以跨运行生长——这是 Scrapy 等工业爬虫的标配设计，留作进阶练习。
